...
router.get("/admin/users", summary="List all users (admin only)", response_model=dict)(get_all_users)
router.patch("/admin/users/{user_id}/status", summary="Update user approval status (admin only)", response_model=dict)(update_user_status)
router.delete("/admin/users/{user_id}", summary="Delete a user (admin only)", response_model=dict)(delete_user)
